...
if (isset($_POST["filter_btn"])) {
  ...
  $query = "SELECT * FROM btechforeign1 WHERE college_name LIKE '%$filter_value%'";
} else {
  $query = "SELECT * FROM btechforeign1";
}
...
if (isset($_POST["filter_apply"])) {
  ...
  if (!empty($filter_location)) {
      $conditions[] = "country = '$filter_location'";
  }
  if (!empty($filter_rank)) {
      $conditions[] = "`qs-rank` = '$filter_rank'";
  }
...
<body>
  <div class="conatiner-xxl pb-4">
    ...
    <div class="image-overlay"><h1> List of top 50 Colleges of B.tech across the World.</h1>
        ...
        <div class="col-lg-2 col-md-2 col-sm-4 pt-4">
            <div class="form-group">
                <select class="form-control outline-primary mx-2" name="filter_location" id="exampleFormControlSelect1">
                    ...
                    <?php
$sql = "SELECT DISTINCT country FROM btechforeign1";
        ...
        <div class="col-lg-2 col-md-2 col-sm-4 pt-4">
            <div class="form-group">
                <select class="form-control outline-primary mx-2" name="filter_rank" id="exampleFormControlSelect1">
                    ...
                    <?php
$sql = "SELECT `qs-rank` FROM btechforeign1";
$result = mysqli_query($con, $sql);
...
if ($result && mysqli_num_rows($result) > 0) {
    ...
    foreach ($rank as $qsrank) { ?>
        <option value="<?php echo $qsrank['qs-rank']; ?>"><?php echo $qsrank['qs-rank']; ?></option>
    <?php }
} else {
    // Error handling if the query fails or no rows are returned
    echo "Not found.";
                            ...
                            <table class="table table-responsive table-striped">
                                ...
                                <tbody>
                                    <?php foreach ($query_run as $student) { ?>
                                        <tr>
                                            <td><?= $student['id']; ?></td>
                                            <td> <a href="test1.php/?College_name=<?= $student['college_name']; ?>"><?= $student['college_name']; ?></a></td>
                                            <td><?= $student['country']; ?></td>
                                            <td><?= $student['city']; ?></td>
                                            <td><?= $student['fees']; ?></td>
                                            
                                            
                                            <td><?= $student['qs-rank']; ?></td>
                                            <td><?= $student['package']; ?></td>
                                           
                                            <td><a href="<?php echo $student['official-link']; ?>">official link</a></td>
                                            <td><a href="<?php echo $student['academic-link']; ?>">academic Detail</a></td>
